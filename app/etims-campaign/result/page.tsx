...
import { taxpayerStore } from "@/app/payments/_lib/store";
import {
	Layout,
	Card,
	Button,
...
import { ResultActions } from "@/app/_components/ResultActions";
import { analytics } from "@/app/_lib/analytics";
import { getKnownPhone } from "@/app/_lib/session-store";

export default function EtimsCampaignResultPage() {
	...
	useEffect(() => {
		...
		analytics.track(
			"etims_campaign_payment_completed",
			{ success: info?.paymentStatus === "success" },
			{ journey_end: true },
		);
		setMounted(true);
	}, []);
	...
	return (
		<Layout
			...
			step="Payment Result"
			onBack={() =>
				router.push(`/etims-campaign/account${phoneParam}`)
			}>
			<div className="space-y-6">
				<Card
					className={`p-6 text-center ${isSuccess ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50"}`}>
					...
					<h2
						className={`text-xl font-bold mb-2 ${isSuccess ? "text-green-800" : "text-red-800"}`}>
						{isSuccess ? "Payment Initiated!" : "Payment Failed"}
					</h2>

					<p
						className={`text-sm ${isSuccess ? "text-green-700" : "text-red-700"}`}>
						{taxpayerInfo.paymentMessage ||
							(isSuccess
				...
				{prn && (
					<Card className="p-4">
						...
						<div className="flex items-center justify-between bg-gray-100 p-3 rounded-lg">
							<span className="font-mono text-lg font-bold text-[var(--kra-red)]">
								{prn}
							</span>
							<button
						...
						<p className="text-xs text-gray-500 mt-2">
							Keep this number for your records. You can use it to
							track your payment.
						</p>
					</Card>
				)}
				...
				<Card className="p-4 space-y-2">
					...
					{taxpayerInfo.taxPeriodFrom ? (
						<IdentityStrip
							...
							value={taxpayerInfo.taxPeriodFrom}
						/>
					) : null}
					{taxpayerInfo.amount != null && taxpayerInfo.amount > 0 ? (
						<IdentityStrip
							label="Amount"
							value={`KES ${taxpayerInfo.amount.toLocaleString()}`}
						/>
					) : null}
				</Card>
				...
				<div className="space-y-3">
					...
					<ResultActions journey="eTIMS Campaign Payment" />
				</div>
			</div>
		</Layout>
	);
