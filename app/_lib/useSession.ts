...
const isPathPublic = (pathname: string | null) => {
	...
	if (
		pathname === "/etims-campaign" ||
		pathname.startsWith("/etims-campaign/validation")
	) {
		return true;
	}
	...
	const checkSession = useCallback(() => {
		...
		if (!isSessionValid()) {
			...
			if (
				...
				finalQuery = queryPrefix
					? `${queryPrefix}&number=${msisdn}`
					: `?number=${msisdn}`;
			}
			...
			const redirectParam = pathname
				...
				: "";
			const phoneParam = msisdn
				? `phone=${encodeURIComponent(msisdn)}`
				: "";
